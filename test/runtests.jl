...
function test1(n)
	...
	a.apprx = prec_roots(a.coeffs)[rand(1:n)]
	calc_precision!(a)
	a = simplify(a)
	@show a.coeffs, convert(Complex{Float64}, a.apprx), a.prec
	b = root(a, 2)
	# b = a*a
	@show b.coeffs, convert(Complex{Float64}, b.apprx), b.prec
	# c = root(b,2)
	c = b * b
	# c = pow2(b)
	@show c.coeffs, convert(Complex{Float64}, c.apprx), c.prec
end

function test2(n)
	coeff = rand(1:10, n + 1)
	a = AlgebraicNumber(coeff, BigFloat(0.0), BigFloat(0.0))
	a.apprx = roots(a.coeffs)[rand(1:n)]
	calc_precision!(a)

	coeff = rand(1:10, n + 1)
	b = AlgebraicNumber(coeff, BigFloat(0.0), BigFloat(0.0))
	b.apprx = roots(b.coeffs)[rand(1:n)]
	calc_precision!(b)

	c = a * b
	abs(roots(c.coeffs) .- c.apprx)
end
